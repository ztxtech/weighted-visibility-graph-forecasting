...
class visibilityGraph:
    ...
    def __init__(self, timeSerie):
        self.timeSerie = torch.Tensor(timeSerie)
        self.length = len(timeSerie)
        self.lowerBound = int(min(timeSerie)) - int(torch.std(self.timeSerie).item())
    ...
    def setVisibilityMatrix(self):
        temp = torch.zeros((self.length, self.length))
    ...
    def setAdjacencyMatrix(self):
        result = torch.zeros((self.length, self.length))
        for i in range(self.length):
            for j in range(i+1,self.length):
    ...
    def setDegreeVector(self):
        self.degreeVector = torch.sum(self.adjacencyMatrix,dim=0)

    def setEdgeNumber(self):
        self.edgeNumber = torch.sum(self.degreeVector).item()

    def setPiMatrix(self):
        result = self.adjacencyMatrix.clone()
        for i in range(self.length):
    ...
    def setPVectorList(self):
        self.pVectorList = torch.zeros((self.length, self.length))
        self.pVectorList[:, 0] = 1

    def setSRW(self, time=500):
        self.setPVectorList()
        self.SRW = torch.zeros((self.length, 1))
        t = 0
        LRW_temp = torch.zeros((self.length, 1))
        while True and t<time:
            t = t + 1
            #p
            for i in range(self.length):
                self.pVectorList[i] = torch.matmul((self.pMatrix.T), self.pVectorList[i])
            #LRW
            LRW = torch.zeros((self.length, 1))
            for i in range(self.length):
    ...
    def setMaxIndex(self):
        index = torch.Tensor(torch.where(self.SRW[:-1] == torch.max(self.SRW[:-1]))[0])
        self.maxIndex = index

    def generateDotList(self):
        dotlist = []
        for idx, v in enumerate(self.timeSerie.tolist()):
    ...
    def generateEdgeList(self):
        ...
        for i in range(self.length-1):
            ...
            for j in range(i, self.length):
                # print(i,j, self.adjacencyMatrix[i][j].item())
                if self.adjacencyMatrix[i][j].item()==1.0:
                    curEdgeList.append([(i+1, self.timeSerie.tolist()[i]),(j+1, self.timeSerie.tolist()[j])])
