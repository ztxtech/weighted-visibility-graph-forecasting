...
plt.title("The process of generating the prediction vector F")
plt.xticks(range(0, len(data) + 1), range(1, len(data) + 2))
for i in range(len(data)):
    plt.bar([i], [data[i]], color=color[i % len(color)], width=0.3, edgecolor='#1B1919FF')
