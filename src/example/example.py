...
import src.vg.visibilityGraph_torch as vg
...
df = pd.read_csv("./data/cci/data.csv")[37:47]
...
for i in range(len(data)):
    plt.bar([i], [data[i]], color=color[i%len(color)] if i==a.maxIndex.item() or i==(len(data)-1) else "grey", width=0.3, edgecolor ='#1B1919FF')

plt.ylim()
i = a.maxIndex.item()
plotline([i, data[i]], [len(data), predicted(i, data[i], len(data) - 1, data[len(data) - 1], len(data))], color1 = color[(i)%(len(color))])
plotline([len(data)-1, data[len(data)-1]], [len(data), predicted(i, data[i], len(data) - 1, data[len(data) - 1], len(data))], color1 = color[(len(data)-1)%(len(color))])
print(predicted(i, data[i], len(data) - 1, data[len(data) - 1], len(data)), end=" ")
plt.axvline(x=len(data),ls="--",c="#012030", lw = 1.5)


plt.savefig("./out/fig/previous.pdf")
plt.show()
