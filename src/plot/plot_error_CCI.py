...
df = pd.read_csv("./out/result/CCI.csv")
date = pd.read_csv("./data/cci/data.csv")['Date'].tolist()[7:]
...
for i in range(5):
    ...
    plt.plot(range(len(errordate)), [j[i] for j in zhanerror], color = color[ci%(len(color))], lw=1.5, path_effects=[pe.Stroke(linewidth=3, foreground='black'), pe.Normal()])
    ci = ci + 1
    plt.plot(range(len(errordate)), [j[i] for j in zhangerror], color = color[ci%(len(color))], lw=1.5, path_effects=[pe.Stroke(linewidth=3, foreground='#676E73'), pe.Normal()])
    ci = ci + 1
    plt.plot(range(len(errordate)), [j[i] for j in maoerror], color = color[ci%(len(color))], lw=1.5, path_effects=[pe.Stroke(linewidth=3, foreground='#676E73'), pe.Normal()])
    ci = ci + 1
    plt.plot(range(len(errordate)), [j[i] for j in arimaerror], color = color[ci%(len(color))], lw=1.5, path_effects=[pe.Stroke(linewidth=3, foreground='#676E73'), pe.Normal()])
    ci = ci + 1
    plt.xticks(range(len(errordate))[::25], errordate[::25], rotation=60)
    plt.legend(["Proposed","Zhang et al.","Mao and Xiao", "ARIMA"])

plt.savefig("./out/fig/CCI_er.pdf")
plt.show()
