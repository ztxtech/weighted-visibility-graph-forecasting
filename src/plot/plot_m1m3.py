...
for idx1,file in enumerate(filename):
    ...
    datasetlist = data_loader.get_dataset("./data/competition/" + file)
    for idx2,data in enumerate(datasetlist):
        ...
        plt.plot(range(len(xlist)),xlist, color = color[index%len(color)],lw=2,path_effects=[pe.Stroke(linewidth=4, foreground='#CCCCCC'), pe.Normal()])
        plt.xticks(size=12)
        index = index + 1
plt.savefig("./out/fig/m1m3.pdf")
