...
df = pd.read_csv("./out/metric/mo_mae.csv")
...
for i in range(len(max_)):
    plt.hlines(name_y[i],min_[i],max_[i],color = color[i%len(color)],lw=3, path_effects=[pe.Stroke(linewidth=3, foreground='#7F7F7F'), pe.Normal()] )
plt.scatter(rank_x,name_y, c ='#1B19197F', s=50, edgecolors="#7F7F7F")



df = pd.read_csv("./out/metric/mo_smape.csv")
methods = df.columns[1:].tolist()
dataname = df['Dataset'].tolist()
...
plt.savefig("./out/fig/m1m3cp.pdf")
plt.show()
